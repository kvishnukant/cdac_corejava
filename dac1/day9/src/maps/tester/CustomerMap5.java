...
public class CustomerMap5 {
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println("Enter Date & disccount amt ");
			SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
			Date d1 = sdf.parse(sc.next());
			double amt = sc.nextDouble();
			// convert map --- Collection<V>
			Collection<Customer> coll = custs.values();
			for (Customer c : coll)
				if (c.getSubscriptionDate().before(d1))
					c.setSubscriptionAmt(c.getSubscriptionAmt() - amt);
			System.out.println("Updated coll " + coll);
