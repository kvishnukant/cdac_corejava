...
public class EmpList4 {
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println("Enter emp id & inc  to update");
			Emp e1 = new Emp(sc.nextInt());
			int index = emps.indexOf(e1);
			if (index == -1)
				throw new EmpNotFoundException("Emp with ID " + e1.getId()
						+ " Not found: Updation failed ");
			double inc = sc.nextDouble();
			Emp e2 = emps.get(index);
			e2.setSal(e2.getSal() + inc);
			System.out.println("Updated List "+emps);
		} catch (Exception e) {
