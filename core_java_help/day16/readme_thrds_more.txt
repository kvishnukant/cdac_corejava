...
6. If a thread goes to sleep(or invokes join,yield,notify) , it holds any locks it has—it doesn't release them.
...
less than a full method—to just a block. OR when u are using Thread un-safe(un-sunchronized) classes in your appln.
...
instance(java.lang.Class<T>)  while the non-static method locks on the "this" instance—these
